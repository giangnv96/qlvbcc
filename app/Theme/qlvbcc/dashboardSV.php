	...
	<div class="container">
		<div class="row">
			...
			if (!empty($tmpVariable['mess'])) {
			 	echo "<h3><strong>".$tmpVariable['mess'] ."</strong></h3>";
			 }
			else
			{
			?>
			<h3>Danh sách văn bằng, chứng chỉ:</h3>
			<?php
			$modelLoai = new Loai();
			foreach ($tmpVariable['list'] as $key => $value) {
			$loaivb = $modelLoai->getDataById($value['Vbcc']['loai']);
			?>

			<a href="/ctVB?id=<?php echo $value['Vbcc']['id'] ?>" title=""><p><span class="glyphicon glyphicon-chevron-right"></span> <?php echo $loaivb['Loai']['name'] ?></p></a>
			<?php
			}
			}
			?>
