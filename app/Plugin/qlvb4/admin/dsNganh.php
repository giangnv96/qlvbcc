...
<table class="table table-bordered table-hover">
	...
	foreach ($listData as $key => $value) {
		$khoa = getKhoaById($value['Nganh']['id_khoa']);
	 	$i++; ?>
	<tr>
		<td><?php echo $i ?></td>
		...
		<td><a href="#" onclick='edit("<?php echo $value['Nganh']['id'] ?>","<?php echo $value['Nganh']['code'] ?>","<?php echo $value['Nganh']['name'] ?>","<?php echo $value['Nganh']['des'] ?>", "<?php echo $value['Nganh']['id_khoa'] ?>")' title="">Sửa</a>|<a href="<?php echo $urlPlugins ?>admin/qlvb-admin-delNganh.php?id=<?php echo $value['Nganh']['id'] ?>" onclick="return confirm('Bạn có chắc chắn muốn xóa không?')"  title="">Xóa</a></td>
	</tr>
	<?php }
	}else
	echo '<tr>
