...
function luuVB()
{
    ...
    if (!empty($_REQUEST['vb'])) {
        ...
        foreach ($_REQUEST['vb'] as $key => $value) {
            ...
            $model->saveMany($save);
        $model->redirect($urlPlugins.'admin/qlvb-admin-dsVB.php?stt=1');
        }
    }
    else
    {
...
function editVB()
{
    ...
    if (!empty($_REQUEST['sohieu'])) {
            ...
            // $save['Vbcc']['masv']=$_REQUEST['code'];
            $save['Vbcc']['loai']=$_REQUEST['loaivb'];
...
function delVB()
{
    ...
    $model->redirect($urlPlugins . 'admin/qlvb-admin-dsVB.php?stt=3');
    }else {
        $model->redirect($urlHomes);
    }
}
