...
function inXacminh()
{
    ...
    if(checkAdminLogin()){
            ...
            if (!empty($_GET['id'])) {
                ...
                $dataVB = (!empty($dataVB))?$dataVB['Vbcc']:'';
...
function taoXacMinh()
{
    ...
    if(checkAdminLogin()){
            ...
            if (!empty($_GET['idxm'])&&!empty($_GET['loai'])) {
                if ($_GET['loai']=="Văn bằng") {
                $dataVB= $modelVB -> find('first',array('conditions'=>array('_id'=>new MongoId($_GET['idxm']))));
                setVariable('dataVB',$dataVB);
                $dataSV= $modelSV -> find('first',array('conditions'=>array('_id'=>new MongoId($dataVB['Vbcc']['id_sv']))));
                setVariable('dataSV',$dataSV);
                }
                elseif ($_GET['loai']=="Chứng chỉ") {
                $dataCC= $modelCC -> find('first',array('conditions'=>array('_id'=>new MongoId($_GET['idxm']))));
                setVariable('dataCC',$dataCC);
                $dataSV= $modelSV -> find('first',array('conditions'=>array('_id'=>new MongoId($dataCC['Vbcc']['id_sv']))));
                setVariable('datSV',$dataSV);
                }
            }
            if (!empty($_REQUEST['submit'])) {
                ...
                $save['Xacminh']['hoten']=(!empty($dataSV['Sinhvien']['hoten']))?$dataSV['Sinhvien']['hoten']:$_REQUEST['hoten'];
                $save['Xacminh']['ngaysinh']=(!empty($dataSV['Sinhvien']['ngaysinh']))?$dataSV['Sinhvien']['ngaysinh']:$_REQUEST['ngaysinh'];
                $save['Xacminh']['gioitinh']=(isset($dataSV['Sinhvien']['gioitinh']))?$dataSV['Sinhvien']['gioitinh']:$_REQUEST['gioitinh'];
                $save['Xacminh']['noisinh']=(!empty($dataSV['Sinhvien']['noisinh']))?$dataSV['Sinhvien']['noisinh']:$_REQUEST['noisinh'];
                $model -> save($save);
                $model->redirect($urlPlugins . 'admin/qlvb-admin-dsXacminh.php?stt=1');
                }
                else
                {
                $save['Xacminh']['donvi']=$_REQUEST['donvi'];
               ...
               $model->redirect($urlPlugins . 'admin/qlvb-admin-dsXacminh.php?stt=2');
                }
            }
    }else {
        $model->redirect($urlHomes);
...
function xoaXacminh()
{
    ...
    if(checkAdminLogin()){
        $id= new MongoID($_REQUEST['id']);
        $model->delete($id);
        $model->redirect($urlPlugins.'admin/qlvb-admin-dsXacminh.php?stt=3');
    }
    else {
...
function dsXacminh()
{
    ...
    if(checkAdminLogin()){
        ...
        setVariable('next',$next);
        setVariable('urlPage',$urlPage);
            $listData = $model -> find('all',array('page'=>$page,'limit'=>$limit));
            setVariable('listData',$listData);
        // }
    }else {
        $model->redirect($urlHomes);
...
function dashboard()
{
    ...
    $conditions['id_sv'] = array('$in'=>$idSV);
        ...
        $loai = explode('-', @$_REQUEST['loai']);
        if ($loai[0]=='Văn bằng'||$loai[0]=='Chứng chỉ') {
            $conditions['loai']=$loai[1];
             $listData1 = $model-> find('all', array('conditions'=>$conditions));
             foreach ($listData1 as $key => $value) {
                 $listData[]=$value['Vbcc'];
             }
...
function delLoaivb()
{
 ...
 include('controller/Hinhthucdt.php');
