			...
			<label>Loại người dùng:</label>
					<select name="loaind" class="form-control" required="">
						...
						foreach ($listPer as $key => $value) {
							if (!empty($data['Manager']['permission'])&&$value['Loaiquyen']['id']==$data['Manager']['permission']) {
						?> <option value="<?php echo $value['Loaiquyen']['id'] ?>" selected=""><?php echo $value['Loaiquyen']['name'] ?></option><?php
						}
						else
						{?>
						<option value="<?php echo $value['Loaiquyen']['id'] ?>"><?php echo $value['Loaiquyen']['name'] ?></option>
						<?php
						}
						}
					} ?>			</select>
